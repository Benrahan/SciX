...
```{r echo=FALSE}
gene.set <- read.gmt('SciX-main/reactome.gmt')
...
```{r echo=FALSE}
disgene <- read.delim('disgenenet.HD.disease.tsv', header=T, sep="\t")
...
### Extract genes from interesting pathway
```{r echo=FALSE}
pathway <- 1
...
plot.data <- plot.data[order(rowSums(plot.data), decreasing = T),][1:10,]
